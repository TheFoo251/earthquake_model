...
def get_loaders(
    img_dir, mask_dir, batch_size, num_workers=4, pin_memory=True, transforms=None
):
    full_ds = EarthquakeDataset(
        image_dir=img_dir, mask_dir=mask_dir, transform=transforms
    )
...
def check_accuracy(loader, model, device="cuda"):
    ...
    print(f"Got {num_correct}/{num_pixels} with acc {num_correct/num_pixels*100:.f}")
